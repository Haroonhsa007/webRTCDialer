...
export default function DashboardPage() {
  ...
  const [activeCallNumber, setActiveCallNumber] = useState<string | undefined>(
    undefined
  );
  ...
  const addCallToLog = useCallback(
    (
      entry: Omit<CallLogEntry, "id" | "durationInSeconds"> & {
      ...
      const duration =
        ...
        (currentCallStartTime
          ? Math.floor((Date.now() - currentCallStartTime) / 1000)
          : 0);
  ...
  const resetCallVisualState = useCallback(() => {
    ...
    if (telnyxClient && callState === "connected") {
      // Check against current state, not client.isConnected
  ...
  const handleCallEnd = useCallback(
    (callEnded: TelnyxCall, finalState: CallState) => {
      console.log(
        `DashboardPage: Handling call end for call ${callEnded.id}, final state: ${finalState}`
      );
      const numberForLog =
        callEnded.remoteCallerNumber ||
        callEnded.options.destinationNumber ||
        "Unknown";
      ...
      const wasActiveOrAnswered =
        ["active", "answering", "early"].includes(callEnded.state as string) ||
        ["active", "answering", "early"].includes(
          callEnded.prevState as string
        ) ||
        currentCallStartTimeVal;

      const callTypeForLog: CallLogEntry["type"] =
        callEnded.direction === "inbound"
          ? wasActiveOrAnswered
            ? "incoming"
            : "missed"
          : "outgoing";

      const duration = currentCallStartTimeVal
        ? Math.floor((Date.now() - currentCallStartTimeVal) / 1000)
        : 0;

      if (
        wasActiveOrAnswered ||
        callTypeForLog === "missed" ||
        (callEnded.direction === "outgoing" &&
          finalState !== "new" &&
          finalState !== "trying" &&
          finalState !== "requesting")
      ) {
        addCallToLog({
        ...
        console.log(
          `DashboardPage: Call ${callEnded.id} not logged. Was active/answered indicator: ${wasActiveOrAnswered}, Type: ${callTypeForLog}, Final State: ${finalState}`
        );
      }

      if (remoteAudioRef.current) {
        console.log(
          `DashboardPage: Clearing remote audio stream for call ${callEnded.id}.`
        );
      ...
      setCurrentCall((prevCall) => {
        if (prevCall && prevCall.id === callEnded.id) {
          console.log(
            `DashboardPage: Clearing currentCall (ID: ${callEnded.id}) due to call end.`
          );
          return null;
        }
        console.warn(
          `DashboardPage: Ended call (ID: ${callEnded.id}) is different from currentCall in state (ID: ${prevCall?.id}). Proceeding with UI reset.`
        );
        return prevCall;
      });
      ...
      if (telnyxClient) {
        // If telnyxClient instance still exists (wasn't nulled by disconnect)
  ...
  const attachCallListeners = useCallback(
    (call: TelnyxCall) => {
      console.log(
        `DashboardPage: Attaching listeners to call ID: ${call.id}, direction: ${call.direction}`
      );
      call.on("telnyx.stream", (streamEvent: any) => {
        console.log(
          `DashboardPage: telnyx.stream event for call ${call.id}. Attaching to remoteAudio element. Stream:`,
          streamEvent.stream
        );
        if (remoteAudioRef.current && streamEvent.stream) {
          ...
          console.error(
            `DashboardPage: remoteAudioRef.current is null or stream is missing. Cannot play remote stream for call ${call.id}. Stream:`,
            streamEvent.stream
          );
        }
      });
      ...
      call.on(
        "telnyx.stateChange",
        (stateChangeEvent: { state: string; prevState: string }) => {
          const telnyxInternalState = stateChangeEvent.state;
          console.log(
            `DashboardPage: telnyx.stateChange for call ${call.id}. Telnyx state: ${telnyxInternalState} (prev: ${stateChangeEvent.prevState})`
          );
          ...
          switch (telnyxInternalState) {
            ...
            case "trying":
              ...
              break;
            case "requesting":
              newUiState = "requesting";
              break;
            case "recovering":
              newUiState = "recovering";
              break;
            case "ringing":
              newUiState = "ringing";
              break;
            case "answering":
              newUiState = "answering";
              break;
            case "early":
              newUiState = "early";
              break;
            ...
            default:
              console.warn(
                `DashboardPage: Unmapped Telnyx call state: ${telnyxInternalState}`
              );
          ...
          if (newUiState === "active" && !callStartTimeRef.current) {
            console.log(
              `DashboardPage: Call ${call.id} became active. Setting callStartTime.`
            );
            const now = Date.now();
          ...
          if (newUiState === "hangup" || newUiState === "destroy") {
            console.log(
              `DashboardPage: Call ${call.id} ended with state ${newUiState}. Triggering handleCallEnd.`
            );
      ...
      call.on("hangup", (params: any) => {
        console.log(
          `DashboardPage: Explicit Call hangup event for call ${call.id}:`,
          params
        );
      ...
      call.on("destroy", () => {
        console.log(
          `DashboardPage: Explicit Call destroy event for call ${call.id}`
        );
  ...
  const setupTelnyxClient = useCallback(() => {
    ...
    if (audioEl) {
      ...
      toast({
        title: "Audio Error",
        description: "Could not find audio output element.",
        variant: "destructive",
        duration: 10000,
      });
    }
    ...
    if (!callerId) {
      ...
      if (!getCookieValue("telnyx_sip_username") || !getCookieValue("telnyx_sip_password")) {
        toast({
          ...
          description: "Please login and set your Caller ID.",
          variant: "destructive",
          duration: 10000,
        });
      }
    ...
    const handleTelnyxReady = () => {
      ...
      console.log("Telnyx Client Ready! client.isConnected property value:", (client as any).isConnected);
    ...
    const handleTelnyxSocketClose = (event: any) => {
      ...
      toast({
        title: "Telnyx Disconnected",
        description: "Connection to Telnyx lost.",
        variant: "destructive",
        duration: 10000,
      });
      setCallState("disconnected");
    ...
    const handleTelnyxNotification = (notification: any) => {
      ...
      if (notification.type === "callUpdate" && notification.call) {
        ...
        if (call.state === "ringing" && call.direction === "inbound") {
          if (currentCall || isReceivingCall) {
            ...
            try {
              ...
              try {
                ...
                console.error("Fallback hangup failed:", e2);
              }
            }
            toast({
              title: "Call Rejected",
              description: "Another call is already in progress or ringing.",
              variant: "destructive",
            });
            return;
          }
          ...
          setCurrentCall(call);
          // Move attachCallListeners call to a separate effect
        ...
        toast({
          title: "Media Error",
          description: notification.error?.message || "Could not access microphone/camera.",
          variant: "destructive",
          duration: 10000,
        });
      }
    ...
    return () => {
      ...
      setCurrentCall((prevCurrentCall) => {
        if (prevCurrentCall && (prevCurrentCall as any).client === client) {
          ...
          try {
            prevCurrentCall.hangup();
          } catch (e) {
            /*ignore errors during cleanup hangup*/
          }
          return null;
        }
  ...
  }, [callerId, toast, currentCall, isReceivingCall]);
  ...
  useEffect(() => {
    const storedCallerId =
      typeof window !== "undefined"
        ? window.localStorage.getItem(CALLER_ID_STORAGE_KEY)
        : null;
    if (storedCallerId) {
      console.log(
        "DashboardPage: Initial mount - Found Caller ID in localStorage:",
        storedCallerId
      );
      ...
      console.warn(
        "DashboardPage: Initial mount - Caller ID not found in localStorage. User input required."
      );
      setIsCallerIdNeeded(true);
  ...
  const handleSaveCallerId = () => {
    ...
    if (trimmedCallerId) {
      if (!trimmedCallerId.startsWith("+") || trimmedCallerId.length < 8) {
        toast({
          title: "Invalid Caller ID Format",
          description:
            "Caller ID should be in E.164 format (e.g., +12345678900).",
          variant: "destructive",
        });
        return;
      }
      console.log(
        "DashboardPage: Saving Caller ID to localStorage:",
        trimmedCallerId
      );
      if (typeof window !== "undefined") {
      ...
      setIsCallerIdNeeded(false);
      toast({
        title: "Caller ID Saved",
        description: "Your Caller ID has been saved.",
      });
    } else {
      toast({
        title: "Invalid Caller ID",
        description: "Please enter a valid Caller ID.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleMakeCall = useCallback(
    async (numberToCall: string) => {
      console.log(
        `DashboardPage: Attempting to make call to ${numberToCall}. Current callState: ${callState}, Telnyx client: ${
          telnyxClient ? "exists" : "null"
        }`
      );
      if (!telnyxClient || callState !== "connected") {
        // Changed condition here
        console.warn(
          "DashboardPage: Cannot make call. Telnyx client is not connected or available."
        );
        toast({
          title: "Not Connected",
          description:
            "Telnyx client is not connected. Please check connection, credentials, and Caller ID.",
          variant: "destructive",
          duration: 10000,
        });
      ...
      if (currentCall || isReceivingCall) {
        console.warn(
          "DashboardPage: Cannot make call. Another call is in progress or ringing."
        );
        toast({
          title: "Call In Progress",
          description:
            "Please end the current call or handle the incoming call.",
          variant: "destructive",
          duration: 7000,
        });
        return;
      ...
      if (!effectiveCallerIdForCall) {
        ...
        toast({
          ...
          description: "Please set your Caller ID before making a call.",
          variant: "destructive",
          duration: 10000,
        });
        setIsCallerIdNeeded(true);
      ...
      console.log(
        `DashboardPage: Creating new Telnyx call to ${numberToCall} from ${effectiveCallerIdForCall}.`
      );
      try {
        ...
        setCallState("new");
        console.log(
          `DashboardPage: New call object created (ID: ${newCall.id}). Call state: 'new'.`
        );
        const now = Date.now();
        callStartTimeRef.current = now;
        setCallStartTime(now);
        ...
        toast({
          ...
          description: (error as Error).message || "Could not start call.",
          variant: "destructive",
        });
        resetCallVisualState();
      }
  ...
  const handleHangup = useCallback(() => {
    if (isReceivingCall && incomingCallDetails?.call) {
      ...
      console.log(
        `DashboardPage: Declining incoming call from ${incomingCallDetails.callerNumber}. Call ID: ${callToDecline.id}`
      );
      try {
    ...
    if (currentCall) {
      ...
      console.log(
        `DashboardPage: Hanging up current call. Call ID: ${callToHangup.id}, State: ${callToHangup.state}`
      );
      try {
        callToHangup.hangup();
      } catch (e) {
        console.warn(
          `DashboardPage: Error hanging up current call (ID: ${callToHangup.id}):`,
          e
        );
      ...
      console.log(
        "DashboardPage: Hangup called but no current call or incoming call to hangup. Resetting state if needed."
      );
      resetCallVisualState();
    }
  }, [
  ...
  const handleMuteToggle = useCallback(() => {
    if (
      currentCall &&
      (currentCall.state === "active" || currentCall.state === "held")
    ) {
      if (isMuted) {
      ...
      console.warn(
        "DashboardPage: Mute toggle called but no active/held call. Current call state:",
        currentCall?.state
      );
    }
  ...
  const handleHoldToggle = useCallback(() => {
    if (currentCall && currentCall.state === "active") {
      console.log(
        `DashboardPage: Putting call on hold. Call ID: ${currentCall.id}`
      );
      currentCall
        .hold()
        .then(() => {
        ...
        .catch((err) => {
          ...
          toast({
            title: "Hold Error",
            description: "Could not put call on hold.",
            variant: "destructive",
          });
        });
    } else if (currentCall && currentCall.state === "held") {
      console.log(
        `DashboardPage: Resuming call from hold. Call ID: ${currentCall.id}`
      );
      currentCall
        .unhold()
        .then(() => {
        ...
        .catch((err) => {
          ...
          toast({
            title: "Resume Error",
            description: "Could not resume call.",
            variant: "destructive",
          });
        });
    } else {
      console.warn(
        "DashboardPage: Hold toggle called but no active/held call. Current call state:",
        currentCall?.state
      );
    }
  }, [currentCall, toast]);
  ...
  const handleAnswerIncomingCall = useCallback(() => {
    if (isReceivingCall && incomingCallDetails?.call) {
      ...
      console.log(
        `DashboardPage: Answering incoming call from ${incomingCallDetails.callerNumber}. Call ID: ${callToAnswer.id}`
      );
      try {
        callToAnswer.answer();
        setIsReceivingCall(false);
        ...
        toast({
          ...
          description: `Connecting to ${
            incomingCallDetails.callerName || incomingCallDetails.callerNumber
          }`,
        });
      } catch (e) {
      ...
      console.warn(
        "DashboardPage: Answer called but no active incoming call details found."
      );
    }
  ...
  const handleSendDtmf = useCallback(
    (digit: string) => {
      if (currentCall && currentCall.state === "active") {
        console.log(
          `DashboardPage: Sending DTMF digit: ${digit} for call ID: ${currentCall.id}`
        );
        try {
          ...
          toast({
            title: "DTMF Error",
            description: "Could not send DTMF tone.",
            variant: "destructive",
          });
        }
      } else {
        console.warn(
          "DashboardPage: Send DTMF called but no active call. Call state:",
          currentCall?.state
        );
        toast({
          title: "DTMF Error",
        ...
        <Card className="w-full shadow-xl">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground text-center">
              Please enter your Telnyx Caller ID number (e.g., +15551234567) to
              enable calling features. This will be stored locally in your
              browser.
            </p>
            <div>
              <Label htmlFor="callerIdInput" className="mb-1 block">
                Caller ID Number
              </Label>
              <Input
                id="callerIdInput"
    ...
    <div className="w-full max-w-lg mx-auto flex flex-col items-center space-y-6">
      ...
      <CallDisplay
        ...
        currentNumber={
          activeCallNumber ||
          (isReceivingCall && incomingCallDetails
            ? incomingCallDetails.callerNumber
            : currentDialNumber)
        }
